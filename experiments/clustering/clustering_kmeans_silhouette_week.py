...
def perform_clustering(data):
    ...
    for cluster_id, data_points_list in cluster_data_points.items():
        ...
        # Print coefficients of the quadratic model
        print('Quadratic model coefficients:')
        print('a:', a, ', b:', b, ', c:', c)

        # Generate data points for quadratic model
        ...
        file_name = f'/Users/roozbeh/Documents/benchmark_code/clustered_results/quadratic_cluster_w{cluster_id + 1}.pdf'
    ...
    pivot_df_subset['Month'] = pivot_df_subset['Date'].dt.month

    # Create a DataFrame to associate each sample with its cluster
    pivot_df_subset['Cluster'] = best_cluster_labels

    # Re-order the columns
    pivot_df_subset = pivot_df_subset[['Date', 'Month', 'File', 'Cluster', 'Skewness', 'Kurtosis']]
    ...
    for cluster_id in range(best_k):
        ...
        plt.yticks(fontsize=30)
        plt.tight_layout()
        file_name = f'/Users/roozbeh/Documents/benchmark_code/clustered_results/clusterrr_w{cluster_id + 1}.pdf'
        plt.savefig(file_name, format='pdf', bbox_inches='tight')
        plt.show() 
