...
def visualize_centroids(cluster_id, centroid, cluster_data_points):
    ...
    plt.grid(axis='y')
    plt.tight_layout()
    file_name = os.path.join(output_path, f'quadratic_cluster_d{cluster_id + 1}.pdf')
...
def visualize_clusters(best_k, best_cluster_labels, scaled_zscore_ema):
    ...
    for cluster_id in range(best_k):
        ...
        plt.yticks(fontsize=30)
        plt.tight_layout()
        file_name = os.path.join(output_path, f'clusterrr_d{cluster_id + 1}.pdf')
        plt.savefig(file_name, format='pdf', bbox_inches='tight')
        plt.show() 
...
def save_results(pivot_df, best_cluster_labels, centroids):
    ...
    pivot_df_subset.to_csv(os.path.join(output_path, 'clustered_kmeans_day.csv'), index=False)

    centroid_df = pd.DataFrame({'Cluster_' + str(i + 1): centroid.ravel() for i, centroid in enumerate(centroids)})
    centroid_df.to_csv(os.path.join(output_path, 'kmeans_centroids_day.csv'), index=False)
